...
static lv_monkey_t* lvgl_monkeys[CONFIG_LV_Z_MAX_MONKEY_COUNT] = {0};
...
static int cmd_lvgl_monkey(const struct shell *sh, size_t argc,
			    char *argv[])
{
    ARG_UNUSED(argc);
    ARG_UNUSED(argv);
    ...
    return SHELL_CMD_HELP_PRINTED;
#else
    shell_error(sh, "Set CONFIG_LV_USE_MONKEY to enable monkey testing support.");
    return -ENOTSUP;
#endif
}

static int cmd_lvgl_monkey_create(const struct shell *sh, size_t argc,
			    char *argv[])
{
#ifdef CONFIG_LV_USE_MONKEY
...
static int cmd_lvgl_monkey_set(const struct shell *sh, size_t argc,
			    char *argv[])
{
#ifdef CONFIG_LV_USE_MONKEY
    int index;
...
SHELL_STATIC_SUBCMD_SET_CREATE(lvgl_cmd_monkey,
    SHELL_CMD_ARG(create, NULL, "Create a new monkey instance (default: pointer)\n"
                                "Usage: lvgl monkey create [pointer|keypad|button|encoder]", cmd_lvgl_monkey_create, 1, 1),
    SHELL_CMD_ARG(set, NULL, "Activate/deactive a monkey instance\n"
